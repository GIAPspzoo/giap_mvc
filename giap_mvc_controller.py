...
from .giap_mvc_model import RoadsModel, AddressPointsModel
...
class GiapMVCController(object):
    ...
    def __init__(self, iface):
        ...
        # self.db_model = RoadsModel()
        self.db_model = AddressPointsModel()
    ...
    def run(self):
        ...
        if self.first_start:
            ...
            self.dlg = GiapMVCView()
            self.road_dlg = RoadView()

            self.connect_listeners()

        self.db_model.set_headers(self.table_model,
                                  self.db_model.cols_headers)
        self.dlg.hide_columns(self.table_model.columnCount(),
                              self.db_model.cols_headers.values())
        self.road_dlg.set_mapper(self.table_model,
                                 self.db_model.cols_headers)
        self.dlg.reset_view(self.table_model)
    ...
    def connect_listeners(self):
        self.dlg.add_btn.clicked.connect(self.add_record)
        self.dlg.edit_btn.clicked.connect(self.edit_record)
        self.dlg.del_btn.clicked.connect(self.del_record)
    ...
    def del_record(self):
        ...
        if rows:
            if QMessageBox.question(
                ...
                return
            self.table_model.removeRow(rows[0].row())
            self.table_model.submitAll()
